...
def test_chebyshev_bound_delegates_to_core(mock_core):
    ...
    result = Utils._chebyshev_bound(variance=1.0, k=2.0)
...
def test_bayes_rule_delegates_to_core(mock_core):
    ...
    result = Utils._bayes_rule(p_B_given_A=0.6, p_A=0.5, p_B=1.0)
...
def test_euclidean_distance_delegates_to_core(mock_core):
    ...
    # Note: Assuming x and y are passed as iterables or C++ representations
    x = [3.0, 0.0]
    y = [0.0, 4.0]
    result = Utils._euclidean_distance(x, y)
    mock_core.euclidean_distance.assert_called_once_with(x, y)
    assert result == 5.0
...
def test_manhattan_distance_delegates_to_core(mock_core):
    ...
    x = [3.0, 0.0]
    y = [0.0, 4.0]
    result = Utils._manhattan_distance(x, y)
    mock_core.manhattan_distance.assert_called_once_with(x, y)
    assert result == 7.0


def test_coefficient_of_variation_delegates_to_core(mock_core):
    mock_core.coefficient_of_variation.return_value = 0.2
    result = Utils._coefficient_of_variation(mean=10.0, stddev=2.0)
...
def test_covariance_delegates_to_core(mock_core):
    ...
    result = Utils._covariance(mean_x=1.0, mean_y=2.0, E_xy=4.5)
...
def test_permutation_delegates_to_core(mock_core):
    ...
    result = Utils._permutation(n=5, k=3)

...
namespace fastdist::cuda::bernoulli {
    ...
    static void bernoulli_cgf_cuda_simple(const double* t, double* output, const int n, const double p, const int stepSize) {
        double *d_t, *d_output;
        ...
        const int offset = 0;
        bernoulli_cgf_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_t, d_output, n, p, stepSize, offset);
    ...
    static void bernoulli_cgf_cuda_streaming(const double* t, double* output, const int n, const double p, const int stepSize) {
        ...
        for (int i = 0; i < STREAM_COUNT; i++) {
            ...
            bernoulli_cgf_kernel<<<blocks, threadsPerBlock, 0, streams[i]>>>(d_t + offset, d_output + offset,
    ...
    void bernoulli_cgf_dispatcher(const double* t, double* output, const int n, const double p, const int stepSize) {
        ...
        if (n < STREAMING_THRESHOLD) {
            bernoulli_cgf_cuda_simple(t, output, n, p, stepSize);
        } else {
            bernoulli_cgf_cuda_streaming(t, output, n, p, stepSize);
        }
    }
} // namespace fastdist::cuda::bernoulli

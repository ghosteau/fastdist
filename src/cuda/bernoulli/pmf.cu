...
namespace fastdist::cuda::bernoulli {
    ...
    static void bernoulli_pmf_cuda_simple(const int* k, double* output, const int n, const double p, const int stepSize) {
        int *d_k;
        double *d_output;
        ...
        const int offset = 0;
        bernoulli_pmf_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_k, d_output, n, p, stepSize, offset);
        ...
        // Ensure kernel excution is complete
    ...
    static void bernoulli_pmf_cuda_streaming(const int* k, double* output, const int n, const double p, const int stepSize) {
        ...
        for (int i = 0; i < STREAM_COUNT; i++) {
            ...
            bernoulli_pmf_kernel<<<blocks, threadsPerBlock, 0, streams[i]>>>(d_k + offset, d_output + offset,
    ...
    void bernoulli_pmf_dispatcher(const int* k, double* output, const int n, const double p, const int stepSize) {
        ...
        if (n < STREAMING_THRESHOLD) {
            bernoulli_pmf_cuda_simple(k, output, n, p, stepSize);
        } else {
            bernoulli_pmf_cuda_streaming(k, output, n, p, stepSize);
        }
    }
} // namespace fastdist::cuda::bernoulli

...
namespace fastdist::cuda::exponential {
    // CUDA kernel
    __global__ void exponential_cdf_kernel(const double* x, double* output, const int n, const double lambda,
    ...
    // Dispatcher
    void exponential_cdf_dispatcher(const double* x, double* output, const int n, const double lambda,
                                    const double stepSize) {
        DeviceContext<double, double>& ctx = get_context<double, double>(n);

        execute_cuda_kernel<double, double>(exponential_cdf_kernel, x, output, ctx.dev_in, ctx.dev_out, n,

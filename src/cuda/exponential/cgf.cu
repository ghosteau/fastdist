...
namespace fastdist::cuda::exponential {
    // CUDA kernel
    __global__ void exponential_cgf_kernel(const double* t, double* output, const int n, const double lambda,
        ...
        if (idx < n) {
            ...
            output[idx] = log(lambda) - log(lambda - t_val);
        }
    }

    // Dispatcher
    void exponential_cgf_dispatcher(const double* t, double* output, const int n, const double lambda,
                                    const double stepSize) {
        DeviceContext<double, double>& ctx = get_context<double, double>(n);

        execute_cuda_kernel<double, double>(exponential_cgf_kernel, t, output, ctx.dev_in, ctx.dev_out, n,

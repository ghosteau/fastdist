...
namespace fastdist::cuda::poisson {
    ...
    static void poisson_cgf_cuda_simple(const double* t, double* output, const int n, const double lambda, const int stepSize) {
        double *d_t, *d_output;
        ...
        const int offset = 0;
        poisson_cgf_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_t, d_output, n, lambda, stepSize, offset);
    ...
    static void poisson_cgf_cuda_streaming(const double* t, double* output, const int n, const double lambda, const int stepSize) {
        ...
        for (int i = 0; i < STREAM_COUNT; i++) {
            ...
            poisson_cgf_kernel<<<blocks, threadsPerBlock, 0, streams[i]>>>(d_t + offset, d_output + offset,
    ...
    void poisson_cgf_dispatcher(const double* t, double* output, const int n, const double lambda, const int stepSize) {
        ...
        if (n < STREAMING_THRESHOLD) {
            poisson_cgf_cuda_simple(t, output, n, lambda, stepSize);
        } else {
            poisson_cgf_cuda_streaming(t, output, n, lambda, stepSize);
        }
    }
} // namespace fastdist::cuda::poisson
